...
module dwallet_system::lc_tests {
    ...
    use dwallet_system::tendermint_lc::{init_consensus_state, Self, init_client, Client};
    ...
    public fun state_proof_test_data(): (vector<u8>, vector<u8>, vector<u8>, vector<u8>, vector<u8>) {
	...
	(proof, prefix, path, value, root)
    }

    fun set_up(ctx: &mut TxContext): (u64, vector<u8>, vector<u8>, vector<u8>, Client) { 
	...
	let (_, _, _, _, client) = set_up(ctx);
        ...
        assert!(tendermint_lc::latest_height(&client) == 6, 0);
	...
	assert!(tendermint_lc::latest_height(&client) == 10, 0);

	let cs = tendermint_lc::get_consensus_state(&client, 10);
        assert!(tendermint_lc::commitment_root(cs) == root, 0);
        assert!(tendermint_lc::timestamp(cs) == timestamp, 0);
        assert!(tendermint_lc::next_validators_hash(cs) == next_validators_hash, 0);
        test_scenario::end(scenario);
