...
export async function createConfigMap(
	...
	for (let i = 0; i < numOfValidators; i++) {
		validatorsConfig[`validator${i + 1}_root-seed.key`] = fs.readFileSync(
			path.resolve(
				TEST_ROOT_DIR,
				`${NETWORK_SERVICE_NAME}.${NAMESPACE_NAME}.svc.cluster.local/val${i + 1}.${NETWORK_SERVICE_NAME}.${NAMESPACE_NAME}.svc.cluster.local/key-pairs/root-seed.key`,
			),
			'utf8',
		);
